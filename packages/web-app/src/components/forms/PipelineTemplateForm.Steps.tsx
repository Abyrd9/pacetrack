...
export function StepsStep({
	...
	}, [fields.step_templates.length, setActiveStepIndex]);
							...
							ref={(el) => {
								if (el) {
									setPipelineStepConfigCards((prev: HTMLDivElement[]) => {
										const exists = prev.find((card) => card.isSameNode(el));
									...
									<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
										<div>
											...
											<NumberInput
												...
												onChange={(v) => step.target_duration_days.onChange(v)}
												placeholder="e.g., 7"
											/>
										</div>

										<div>
											<Label htmlFor={step.icon.name}>Icon</Label>
											<Input
												id={step.icon.name}
												name={step.icon.name}
												value={step.icon.value}
												onChange={(e) => step.icon.onChange(e.target.value)}
												placeholder="CircleDot"
											/>
										</div>

										<div>
											<Label htmlFor={step.color.name}>Color</Label>
											<div className="flex gap-2">
												<Input
													...
													onChange={(e) => step.color.onChange(e.target.value)}
