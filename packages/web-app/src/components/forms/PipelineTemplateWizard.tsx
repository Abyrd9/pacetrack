...
export function PipelineTemplateWizard({
	...
	const handlePreviewStepClick = (index: number) => {
		...
		// Now scroll to the config card
		// Use setTimeout to ensure refs are populated
		setTimeout(() => {
			const element = pipelineStepConfigCards[index];
			console.log("Scrolling to step", index, "element:", element);
			if (element) {
				// Get the element's position
				const elementRect = element.getBoundingClientRect();
				const absoluteElementTop = elementRect.top + window.scrollY;

				// Calculate offset to account for sticky footer (approximately 360px)
				// We want the element centered in the visible area above the sticky footer
				const stickyFooterHeight = 360;
				const availableViewportHeight = window.innerHeight - stickyFooterHeight;
	...
	// Scroll preview to active step
	useEffect(() => {
		if (activeStepIndex !== null && pipelineDrawnSteps[activeStepIndex]) {
			pipelineDrawnSteps[activeStepIndex]?.scrollIntoView({
				behavior: "smooth",
				block: "nearest",
				inline: "center",
			});
	...
	// Scroll to config card when step changes to 2 and there's an active index
	useEffect(() => {
		if (currentStep === 2 && activeStepIndex !== null) {
			// Use setTimeout to ensure the DOM has updated
			setTimeout(() => {
				const element = pipelineStepConfigCards[activeStepIndex];
				if (element) {
					const elementRect = element.getBoundingClientRect();
					const absoluteElementTop = elementRect.top + window.scrollY;
					...
					const availableViewportHeight =
						window.innerHeight - stickyFooterHeight;
					const scrollPosition =
						absoluteElementTop -
						availableViewportHeight / 2 +
						elementRect.height / 2;
