...
export function ManageTenantsForm({
	...
	const { mutate: onSubmit, isPending } = useMutation({
		mutationFn: async (form: FormData) => {
			const { data } = await client<TenantDeleteRouteResponse>(
				TENANT_DELETE_ROUTE_PATH,
				{
					method: "POST",
					body: form,
		...
		onSuccess: (data, variables) => {
			if (data.status === "ok") {
				...
				qc.setQueryData([TENANT_GET_ROUTE_PATH], (old: Tenant[]) =>
					old.filter((t) => t.id !== tenantToDeleteId),
				);
				qc.invalidateQueries({ queryKey: [TENANT_GET_ROUTE_PATH] });
				qc.invalidateQueries({ queryKey: [ACCOUNT_GET_ROLES_ROUTE_PATH] });
			}
		},
	});
