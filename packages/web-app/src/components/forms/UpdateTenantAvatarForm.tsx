import {
	...
	type Tenant,
	type TenantUpdateRouteResponse,
...
export function UpdateTenantAvatarForm({
	...
	const {
		...
		mutationFn: async (form: FormData) => {
			const { data } = await client<TenantUpdateRouteResponse>(
				TENANT_UPDATE_ROUTE_PATH,
				{
					method: "POST",
					body: form,
		...
		onSuccess: (data) => {
			if (data.status === "ok") {
				...
				qc.invalidateQueries({
					queryKey: [TENANT_GET_BY_ID_ROUTE_PATH, tenant?.id],
				});

				if (tenant?.id === context.tenant?.id) {
					router.invalidate();
				}
			}
		},
