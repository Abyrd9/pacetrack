...
describe("Tenant Deletion Validation", () => {
  describe("checkTenantDeletionBlockers", () => {
    ...
    test("should block deletion if tenant has active subscription", async () => {
      await resetDb();
      const user = await setTestSession();

      // Create org tenant (has subscription by default from createTestTenant)
      // createTestTenant automatically adds user to tenant
      const orgTenant = await createTestTenant(
        user.user.id,
        "Company",
        user.account.id
      ...
      const subscriptionBlocker = blockers.find(
        (b) => b.type === "active_subscription"
      );
      expect(subscriptionBlocker).toBeDefined();
      expect(subscriptionBlocker?.message).toContain("subscription");
    });
    ...
    test("should allow deletion of org tenant with no blockers", async () => {
      ...
      // Create org tenant without subscription
