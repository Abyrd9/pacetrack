...
class RateLimiter {
  ...
  async consume(key: string, cost: number = 1): Promise<boolean> {
    ...
    try {
      ...
      if (isRedisConnectionError(error)) {
        console.error(
          "Token bucket Redis connection error:",
          error instanceof Error ? error.message : String(error)
        );

        // If configured to fail closed during Redis failures, deny the request
        if (this.config.failClosed) {
          console.warn(
            "Rate limiter failing closed due to Redis connection failure"
          );
          return false;
        }

        // Otherwise fail open (allow the request)
        console.warn(
          "Rate limiter failing open due to Redis connection failure"
        );
        return true;
      }

      // For other Redis errors (script loading, etc.), log and fail open
  ...
  async getBucketInfo(
    ...
    try {
      ...
      if (isRedisConnectionError(error)) {
        console.error(
          "Error getting bucket info - Redis connection failed:",
          error instanceof Error ? error.message : String(error)
        );
      } else {
