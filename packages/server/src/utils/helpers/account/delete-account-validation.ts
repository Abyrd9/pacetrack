...
export type AccountDeletionBlocker = {
  type: "sole_owner" | "billing_owner" | "personal_tenant" | "last_account";
  tenantId?: string;
  tenantName?: string;
  message: string;
};
...
export async function checkAccountDeletionBlockers(
  ...
  for (const metadata of accountTenants) {
    ...
    if (tenant.kind === "org") {
      ...
      // Check if this account created the tenant's membership (billing owner)
      const tenantWithMembership = await db.query.tenant_table.findFirst({
      ...
      if (tenantWithMembership?.membership.created_by === userId) {
        blockers.push({
          type: "billing_owner",
          tenantId: tenant.id,
          tenantName: tenant.name,
          message: `You are the billing owner of "${tenant.name}". Transfer billing ownership or cancel the subscription first.`,
        });
      }
    }
  }
