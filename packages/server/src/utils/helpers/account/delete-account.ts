...
export async function deleteUserEntirely(userId: string): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    ...
    // Delete all personal tenants and their memberships
    for (const tenant of personalTenants) {
      // Cancel Stripe subscription before soft-deleting
      if (tenant.membership.subscription_id) {
        await cancelSubscription(tenant.membership.subscription_id);
      }
      ...
      await tx
        ...
        .where(eq(membership_table.id, tenant.membership_id));
