...
describe("Account Deletion Validation", () => {
  describe("checkAccountDeletionBlockers", () => {
    ...
    test("should block deletion if account is billing owner", async () => {
      await resetDb();
      const user = await setTestSession();
      ...
      // Create org tenant (user is billing owner because they created the membership)
      // createTestTenant automatically adds account to tenant
      const orgTenant = await createTestTenant(
        user.user.id,
        "Company",
        user.account.id
      ...
      const billingBlocker = blockers.find((b) => b.type === "billing_owner");
      expect(billingBlocker).toBeDefined();
      expect(billingBlocker?.tenantName).toBe("Company");
    });
    ...
    test("should allow deletion if account is member (not owner) of org tenant", async () => {
      ...
      // Should only have "last_account" blocker, not sole_owner or billing_owner
      const soleOwnerBlocker = blockers.find((b) => b.type === "sole_owner");
      const billingBlocker = blockers.find((b) => b.type === "billing_owner");

      expect(soleOwnerBlocker).toBeUndefined();
      expect(billingBlocker).toBeUndefined();
    });
  });
