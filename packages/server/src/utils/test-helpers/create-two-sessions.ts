...
export async function createTwoSessions(): Promise<TwoSessions> {
  ...
  const { sessionId, sessionSecretHash } =
    await getSessionClient().createSessionToken();

  await getSessionClient().create({
    sessionId,
    sessionSecretHash,
    userId: user.id,
  ...
  const updatedUserSessions = await getSessionClient().listUserSessions({
    userId: user.id,
  });
