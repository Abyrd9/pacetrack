...
describe("Reset Password Validate Route", () => {
  test("should return 400 if email is not provided", async () => {
    const response = await app.request(RESET_PASSWORD_VALIDATE_ROUTE.path, {
      method: "POST",
      body: JSON.stringify({
        code: "test-code",
      }),
  ...
  test("should return 400 if user is not found", async () => {
    ...
    const body = await response.json();
    expect(body.status).toBe("error");
    expect(body.errors.email).toBe("Account not found");
  });

  test("should return 400 if code is invalid", async () => {
    // Create a test user and account with a reset password token
  ...
  test("should return 400 if code is expired", async () => {
    ...
    // Create a test user and account with an expired reset token
