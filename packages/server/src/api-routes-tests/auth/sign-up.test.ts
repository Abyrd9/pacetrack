...
describe("Sign Up Route", () => {
  test("should return 400 if email is not provided", async () => {
    ...
    const response = await app.request(SIGN_UP_ROUTE.path, {
      method: "POST",
      body: form,
    });
  ...
  // Test for successful signup with account, tenant, and role creation
  test("Should create account, tenant, and role on successful signup", async () => {
    const form = new FormData();
    ...
    const getTenantAndRole = await db.query.account_metadata_table.findFirst({
