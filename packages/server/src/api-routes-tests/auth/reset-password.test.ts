...
beforeAll(async () => {
  ...
  // Create user first
  const [user1] = await db.insert(user_table).values({}).returning();
  ...
  const [user2] = await db.insert(user_table).values({}).returning();
...
describe("Reset Password Route", () => {
  test("should return 400 if email is not provided", async () => {
    ...
    const response = await app.request(RESET_PASSWORD_ROUTE.path, {
      method: "POST",
      body: form,
    });
  ...
  test("should return 200 if password reset is successful", async () => {
    ...
    const account = await db.query.account_table.findFirst({
      where: eq(account_table.email, "reset@example.com"),
    });

    expect(account).toBeTruthy();
    expect(account?.reset_password_token).toBeNull();
    expect(account?.reset_password_expires).toBeNull();

    // Verify that the password was updated (can sign in with new password)
    const isPasswordUpdated =
      account?.password &&
      (await Bun.password.verify("newpassword123", account.password));
