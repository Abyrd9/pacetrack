...
describe("Account Link Route", () => {
  ...
  test("adds account with multiple team tenants to active_accounts", async () => {
    ...
    const otherMembership = await db.query.membership_table.findFirst({
      where: (m, { eq }) => eq(m.created_by, otherUser.id),
    });
    if (!otherMembership) throw new Error("Other membership not found");

    // Create two additional org tenants manually
    const [teamTenant1] = await db
      .insert(tenant_table)
      .values({
        name: "Team 1",
        kind: "org",
    ...
    // Link the account to both team tenants
    ...
    await db.insert(account_metadata_table).values([
      {
        ...
        tenant_id: teamTenant1.id,
        role_id: role.id,
      },
      {
        user_id: otherUser.id,
        account_id: otherAccount.id,
        tenant_id: teamTenant2.id,
        role_id: role.id,
      },
    ]);
    ...
    if (sessionId) {
      ...
      const hasTeam1 = session?.active_accounts.some(
        (acc) =>
          acc.account_id === otherAccount.id && acc.tenant_id === teamTenant1.id
      );
      const hasTeam2 = session?.active_accounts.some(
        (acc) =>
          acc.account_id === otherAccount.id && acc.tenant_id === teamTenant2.id
      );
