...
describe("Session Switch Account Route", () => {
	test("successfully switches to another account for the same user", async () => {
		...
		const response = await app.request(SESSION_SWITCH_ACCOUNT_ROUTE_PATH, {
			method: "POST",
	...
	test("can switch between multiple accounts for the same user", async () => {
		...
		const response1 = await app.request(SESSION_SWITCH_ACCOUNT_ROUTE_PATH, {
			method: "POST",
			headers: makeAuthenticatedRequest(cookie, csrfToken, "POST", {
				"Content-Type": "application/json",
			}),
		...
		const response2 = await app.request(SESSION_SWITCH_ACCOUNT_ROUTE_PATH, {
			method: "POST",
			headers: makeAuthenticatedRequest(cookie, csrfToken, "POST", {
				"Content-Type": "application/json",
			}),
		...
		const response3 = await app.request(SESSION_SWITCH_ACCOUNT_ROUTE_PATH, {
			method: "POST",
			headers: makeAuthenticatedRequest(cookie, csrfToken, "POST", {
				"Content-Type": "application/json",
			}),
