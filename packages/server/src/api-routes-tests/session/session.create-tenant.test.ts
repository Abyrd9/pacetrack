...
describe("Session Create Tenant Route", () => {
  test("successfully creates a new tenant and switches session to it", async () => {
    ...
    // Verify membership was created for this tenant
    expect(tenant?.membership_id).toBeDefined();
    const membership = await db.query.membership_table.findFirst({
      where: (m, { eq }) => eq(m.id, tenant.membership_id),
    });
  ...
  test("creates tenant with existing membership when membership_id is provided", async () => {
    await resetDb();
    const { account, membership, cookie, csrfToken } = await setTestSession();

    // Use the membership from the test session (user already has billing permissions)
    const form = new FormData();
    form.append("name", "Test Workspace");
    form.append("account_id", account.id);
    ...
    // Verify tenant uses the provided membership
    const tenant = await db.query.tenant_table.findFirst({
      where: (tenants, { eq }) => eq(tenants.name, "Test Workspace"),
    });
    expect(tenant).toBeDefined();
