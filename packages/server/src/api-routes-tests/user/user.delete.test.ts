...
describe("User Delete Route", () => {
  test("should delete user with single account and personal tenant", async () => {
    ...
    const deletedMembership = await db.query.membership_table.findFirst({
      where: eq(membership_table.id, user.membership.id),
    });
    expect(deletedMembership?.deleted_at).toBeDefined();
  ...
  test("should delete user with multiple accounts", async () => {
    ...
    const [membership2] = await db
      ...
      .values({
        ...
        subscription_id: "sub_mock_test2",
      })
      .returning();
  ...
  test("should delete all account groups memberships", async () => {
    await resetDb();
    const user = await setTestSession();

    // Create account group in personal tenant (not org tenant to avoid sole owner blocker)
    const [group] = await db
      .insert(account_group_table)
      .values({
        name: "Engineering",
        tenant_id: user.tenant.id, // Use personal tenant
      })
      .returning();
    ...
    // Verify account-to-group relationship was soft-deleted
    const deletedRelation =
      await db.query.account_to_account_group_table.findFirst({
        where: and(
          eq(account_to_account_group_table.account_id, user.account.id),
          eq(account_to_account_group_table.account_group_id, group.id)
        ),
      });
    expect(deletedRelation?.deleted_at).toBeDefined();
  });
