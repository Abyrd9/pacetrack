...
describe("User Change Email Route", () => {
	test("returns 401 when unauthenticated (CSRF token required)", async () => {
		const response = await app.request(CHANGE_EMAIL_ROUTE_PATH, {
			method: "POST",
		});
	...
	test("returns 400 when email already taken", async () => {
		...
		const existingUser = await createTestUser(tenant.id, "existing@test.com");

		const form = new FormData();
		form.append("email", existingUser.email);
