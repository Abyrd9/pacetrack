...
describe("User Confirm Email Change Route", () => {
  test("returns 400 when required fields missing", async () => {
    const response = await app.request(CONFIRM_EMAIL_CHANGE_ROUTE_PATH, {
      method: "POST",
  ...
  test("returns 400 when token invalid", async () => {
    ...
    await app.request(CHANGE_EMAIL_ROUTE_PATH, {
      method: "POST",
      body: form,
  ...
  test("successfully confirms email change", async () => {
    ...
    // Initiate change
    const newEmail = `final${user.email}`;
    const form = new FormData();
    form.append("email", newEmail);
    const initiateResp = await app.request(CHANGE_EMAIL_ROUTE_PATH, {
      method: "POST",
      body: form,
      headers: makeAuthenticatedRequest(cookie, csrfToken, "POST"),
    });
    ...
    const row = await db.query.user_table.findFirst({
      where: eq(user_table.id, user.id),
    });
    ...
    const updated = await db.query.user_table.findFirst({
      where: eq(user_table.id, user.id),
    });
