...
export function accountGroupGetRoute(app: App) {
  app.post(ACCOUNT_GROUP_GET_ROUTE.path, async (c) => {
    try {
      const accountId = c.get("account_id");
      const tenantId = c.get("tenant_id");

      const parsed = await getParsedBody(c.req, ACCOUNT_GROUP_GET_ROUTE.request);

      if (!parsed.success) {
        return c.json(
          ACCOUNT_GROUP_GET_ROUTE.createRouteResponse({
            status: "error",
