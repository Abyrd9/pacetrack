...
export function accountQueryRoute(app: App) {
	app.post(ACCOUNT_QUERY_ROUTE_PATH, async (c) => {
		try {
			const tenantId = c.get("tenant_id");

			const parsed = await getParsedBody(c.req, AccountQueryRequestSchema);
			if (!parsed.success) {
				return c.json(
					makeAccountQueryRouteResponse({
						key: ACCOUNT_QUERY_ROUTE_PATH,
						status: "error",
