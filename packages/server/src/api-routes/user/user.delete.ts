...
export function userDeleteRoute(app: App) {
	app.post(USER_DELETE_ROUTE_PATH, async (c) => {
		...
		logger.middleware("USER_DELETE", "Starting user delete request", requestId);

		try {
			logger.middleware("USER_DELETE", "Parsing request body", requestId);
			...
			logger.middleware("USER_DELETE", "Checking if user exists", requestId);
			...
			logger.middleware("USER_DELETE", "User deleted successfully", requestId, {
				userId: parsed.data.userId,
			});
